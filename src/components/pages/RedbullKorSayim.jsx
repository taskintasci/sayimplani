...
export default function RedbullKorSayim({ onNavigate }) {
  ...
  const filterOptions = useMemo(
    () => computeFilterOptionsRedbull(korMatched, { filterSearch, filterDurum, filterBina, filterBlok, filterKoridor, filterSutun, filterKat }),
    [korMatched, filterSearch, filterDurum, filterBina, filterBlok, filterKoridor, filterSutun, filterKat]
  )
  ...
  }, [korMatched, filterSearch, filterDurum, filterBina, filterBlok, filterKoridor, filterSutun, filterKat, sortType])
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {korMatched.length > 0 && (
        <div className="px-5 py-2 bg-white border-t border-slate-200 flex items-center justify-between shrink-0 no-print">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => exportRedbullResults(korMatched, results, session, firmaProfile)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-[12.5px] font-medium text-slate-700 hover:bg-slate-50">
      ...
      <div className="hidden">
        <RedbullPrintSheet ref={printRef} rows={filtered} results={results} session={session} mode="kor" hideSistem={hideSistem} hideSayilan={hideSayilan} sayimTuru="Kör Sayım" firmaUnvani={firmaProfile?.unvan} />
      </div>
