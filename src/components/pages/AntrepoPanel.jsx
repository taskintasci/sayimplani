...
import { exportAntrepoResults, exportAntrepoRaporFarklar } from '../../utils/excelExport'
...
export default function AntrepoPanel({ onNavigate }) {
  ...
  async function handleClearRows() {
    if (counted.length > 0) {
      await exportAntrepoRaporFarklar(discrepancies, session, manualRows, firmaProfile)
    }
    await clearRows()
          ...
          {userRole !== 'kontrolcu' && (
            rows.length === 0 ? (
              <button onClick={() => onNavigate('upload')} className="bg-white rounded-xl border border-slate-200 p-4 text-left hover:border-blue-300 hover:shadow-md hover:-translate-y-0.5 transition-all group">
                ...
                <p className="text-[13.5px] font-semibold text-slate-800">WMS_Rapor_31 Yükle</p>
              ...
              <div className="bg-white rounded-xl border border-green-200 p-4 text-left">
                ...
                <p className="text-[13.5px] font-semibold text-slate-800">WMS_Rapor_31 Yönetimi</p>
                <p className="text-[12px] text-slate-400 mt-0.5">{rows.length.toLocaleString('tr')} kalem · {importFormat === 'wms31' ? 'WMS_Rapor_31' : importFormat === 'rapor5' ? 'RAPOR5' : importFormat === 'sku' ? 'SKU Listesi' : 'Bilinmiyor'}</p>
