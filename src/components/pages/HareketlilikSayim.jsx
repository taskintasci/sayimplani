...
function girisGunStyle(gun) {
  const g = Number(gun)
  if (!gun || isNaN(g) || g === 0) return {}
  if (g <= 30)  return {}
  if (g <= 90)  return { background: 'rgba(255,251,235,0.9)' }
  if (g <= 180) return { background: 'rgba(254,249,195,0.9)' }
  return { background: 'rgba(254,226,226,0.9)' }
}

function GirisGunBadge({ gun }) {
  const g = Number(gun)
  if (!gun || isNaN(g)) return <span className="text-slate-300">—</span>
  let cls = 'text-slate-500'
  if (g > 180)      cls = 'text-red-500 font-semibold'
  else if (g > 90)  cls = 'text-amber-600 font-medium'
  else if (g > 30)  cls = 'text-yellow-600'
  return <span className={cls + ' mono text-[11.5px]'}>{g}</span>
}
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-5 pt-3 pb-2 bg-white border-b border-slate-200 shrink-0">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-[15px] font-bold text-slate-900">Hareketlilik Sayımı</h1>
          ...
          <table className="w-full text-left border-collapse" style={{ minWidth: 1180 }}>
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-800 text-white text-[11px] mono uppercase tracking-wider">
                ...
                <th className="px-3 py-2.5 w-16 text-right text-emerald-300">G.Gün</th>
              ...
              {paginated.map((row, localI) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right"><GirisGunBadge gun={row.girisGun} /></td>
