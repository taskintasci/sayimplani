...
import { exportRedbullResults, exportRedbullRaporFarklar } from '../../utils/excelExport'
...
export default function RedbullPanel({ onNavigate }) {
  ...
  // Panel'den "Tüm Stok" yazdırma/Excel — sayfa/filtre seçimi olmadığı için
  // her zaman TÜM satırlar (rows). Sayım sayfalarındaki mevcut PrintSheet/
  // exportRedbullResults deseni aynen kullanılıyor; manuel eklenen kalemler
  // (sistemde bulunmayanlar) sayfanın/Excel'in EN SONUNA eklensin diye
  // sentetik bir "sistem 0, sayılan miktar" satırı+sonucu olarak `rows`/
  // `results`'a birleştiriliyor (kullanıcı isteği) — PrintSheet bileşeninin
  // kendisine dokunmaya gerek kalmadı.
  const printRef = useRef()
  const handlePrint = useReactToPrint({ contentRef: printRef })
  const manualAsRows = useMemo(
    () => manualRows.map(m => ({ id: m.id, adres: m.adres, kod: m.kod, ad: m.ad, sscc: m.sscc, parti: m.parti, durum: m.durum, birim: m.birim, sayim: 0 })),
    [manualRows]
  )
  ...
  async function handleClearRows() {
    if (counted.length > 0) {
      await exportRedbullRaporFarklar(discrepancies, session, manualRows, firmaProfile)
    }
    await clearRows()
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-2">
        <div>
          <h1 className="text-xl font-bold text-slate-900">WMS Depo Sayım (Redbull) Paneli</h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={() => exportRedbullResults(rows, results, session, firmaProfile, manualRows)}
      ...
      <div>
        ...
        <div className={`grid gap-3 ${userRole === 'kontrolcu' ? 'grid-cols-1 max-w-xs' : 'grid-cols-3'}`}>
          {userRole !== 'kontrolcu' && (
            rows.length === 0 ? (
              <button onClick={() => onNavigate('upload')} className="bg-white rounded-xl border border-slate-200 p-4 text-left hover:border-blue-300 hover:shadow-md hover:-translate-y-0.5 transition-all group">
                ...
                <p className="text-[13.5px] font-semibold text-slate-800">WMS Depo Redbull Yükle</p>
              ...
              <div className="bg-white rounded-xl border border-green-200 p-4 text-left">
                ...
                <p className="text-[13.5px] font-semibold text-slate-800">WMS Depo Redbull Yönetimi</p>
                <p className="text-[12px] text-slate-400 mt-0.5">{rows.length.toLocaleString('tr')} kalem · {importFormat === 'redbull' ? 'WMS Depo Redbull' : importFormat === 'wms31' ? 'WMS_Rapor_31' : importFormat === 'rapor5' ? 'RAPOR5' : importFormat === 'sku' ? 'SKU Listesi' : 'Bilinmiyor'}</p>
          ...
          {userRole !== 'kontrolcu' && (
            <button onClick={() => onNavigate('redbullkor')} className="bg-white rounded-xl border border-slate-200 p-4 text-left hover:border-blue-300 hover:shadow-md hover:-translate-y-0.5 transition-all group">
          ...
          <button onClick={() => onNavigate('redbullrapor')} className="bg-white rounded-xl border border-slate-200 p-4 text-left hover:border-blue-300 hover:shadow-md hover:-translate-y-0.5 transition-all group">
      ...
      {rows.length > 0 && (
        <div className="flex justify-end">
          <button onClick={() => onNavigate('redbullsayim')} className="flex items-center gap-1.5 text-[13px] font-semibold text-blue-600 hover:text-blue-700">
            Sayıma Devam Et
      ...
      <div className="hidden">
        <RedbullPrintSheet ref={printRef} rows={printRows} results={printResults} session={session} mode="sayim" sayimTuru="Tüm Stok" firmaUnvani={firmaProfile?.unvan} />
      </div>
    </div>
  )
}
