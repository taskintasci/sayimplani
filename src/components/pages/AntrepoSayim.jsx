...
export default function AntrepoSayim({ onNavigate }) {
  ...
  const filterOptions = useMemo(
    () => computeFilterOptions(rows, { filterSearch, filterDurum, filterKategori, filterUrunTipi, filterBlok, filterKoridor, filterSutun, filterKat }),
    [rows, filterSearch, filterDurum, filterKategori, filterUrunTipi, filterBlok, filterKoridor, filterSutun, filterKat]
  )
  ...
  }, [rows, filterSearch, filterDurum, filterKategori, filterUrunTipi, filterBlok, filterKoridor, filterSutun, filterKat, sortType])
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {rowsLoading ? (
          ...
          <table className="w-full text-left border-collapse" style={{ minWidth: 1500 }}>
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-800">
                {[8, 96, 112, 200, 100, 70, 90, 130, 60, 80, 80, 90, 90, 60, 120].map((w, i) => (
            ...
            <tbody>
              {Array.from({ length: 18 }).map((_, i) => (
                <tr key={i} className="border-b border-slate-100" style={i % 2 === 1 ? { background: '#f8fafc' } : {}}>
                  {[8, 96, 112, 200, 100, 70, 90, 130, 60, 80, 80, 90, 90, 60, 120].map((w, j) => (
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-slate-50">
          ...
          <div className="text-slate-400 text-[13px] mb-4">WMS_Rapor_31.xlsx yükleyin</div>
      ...
      {rows.length > 0 && (
        <div className="px-5 py-2 bg-white border-t border-slate-200 flex items-center justify-between shrink-0 no-print">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => exportAntrepoResults(rows, results, session, firmaProfile)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-[12.5px] font-medium text-slate-700 hover:bg-slate-50">
      ...
      <div className="hidden">
        <AntrepoPrintSheet ref={printRef} rows={filtered} results={results} session={session} mode="sayim" hideSayilan={hideSayilan} sayimTuru="Stok Sayımı" firmaUnvani={firmaProfile?.unvan} />
      </div>
      ...
      {gorevModal && (
        <GorevAtaModal
          ...
          sayimTipi="antrepo"
          filtreOzeti={buildFiltreOzeti({ filterSearch, filterDurum, filterKategori, filterUrunTipi, filterBlok, filterKoridor, filterSutun, filterKat })}
        />
      )}
    </div>
  )
