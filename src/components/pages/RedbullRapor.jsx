...
export default function RedbullRapor({ onNavigate }) {
  ...
  // Rapordaki mevcut SSCC'lerden yazdıkça öneri — serbest metin, listede
  // olmayan bir SSCC de girilebilir (ilk 7-8 hane genelde aynı olduğundan
  // öneri yazarken yön gösterici olması yeterli, doğrulama zorunlu değil).
  ...
  return (
    <div className="flex flex-col gap-5 print-content">
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-2">
        <div>
          <h1 className="text-xl font-bold text-slate-900">{firmaProfile?.unvan || firmaProfile?.ad || 'WMS Depo Sayım (Redbull)'} Mutabakat Raporu</h1>
        ...
        <div className="flex flex-wrap gap-2 no-print">
          ...
          <button
            onClick={() => exportRedbullRaporFarklar(discrepancies, session, manualRows, firmaProfile)}
      ...
      {discrepancies.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <div className="text-[13px] text-slate-400 mt-1">
            {counted.length === 0
              ? 'Henüz sayım yapılmamış. Tüm Stok sayfasından başlayın.'
                ...
                {visibleDiscrepancies.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 1 ? 'bg-slate-50/50 hover:bg-slate-50' : 'hover:bg-slate-50'}>
                    ...
                    <td className="px-3 py-1.5 text-center no-print">
                      ...
                      <button
                        onClick={() => { setPendingKodFilter(row.kod); onNavigate('redbullsayim') }}
